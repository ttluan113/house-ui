...
import styles from './CardBodyPayment.module.scss';
...
import { requestGetUserById, requestGetHouseHeart, requestHeartHouse, requestPaymentsMomo } from '../../Config';
...
function CardBodyPayment({ house, isFavorite }) {
    ...
    const handlePaymentsHouse = async (postId) => {
        try {
            const res = await requestPaymentsMomo(postId);
            window.open(res);
        } catch (error) {}
    };
        ...
        <div className={cx('wrapper')} id={cx(house.charged === 1 ? 'border-charged' : '')}>
            ...
            <div className={cx('info-home')}>
                ...
                <div className={cx('info-home')}>
                    ...
                    <div>
                        <Button
                            onClick={() => handlePaymentsHouse(house?.postId)}
                            id={cx('btn-buy')}
                            variant="contained"
                        >
                            Thanh Toán Ngay
...
export default CardBodyPayment;
